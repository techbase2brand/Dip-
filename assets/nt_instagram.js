    ...
    geckoShopify.refresh_ins = function (_this) {
      ...
      if ( data != null && data != '' ) {
           ...
           $.ajax({
             ...
             error: function (e) {
              console.error("Instagram Feed:error time1");
              ...
              fetch('https://graph.instagram.com/refresh_access_token?grant_type=ig_refresh_token&access_token='+acc)
                ...
                .then(function(media) {
                  ...
                  ins_fetch(url,_this,acc,limit,dtid);

                })
                .catch(function(err) {
                  //alert(err);
                  console.error("Instagram Feed:error refresh_token");
                  ins_fetch(url,_this,acc,limit,dtid);
...
//               });
    ...
    function ins_fetch(url,_this,acc,limit,dtid) {
      fetch(ajaxUrl)
        .then(function(response) {
          if(!response.ok) {
            throw new Error("not ok");
